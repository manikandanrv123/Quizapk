...
function Admin() {
  ...
  const handleAddQuestion = () => {
    ...
    const {data} = axios.post("http:localhost:8080/Question",newQuestion,{ headers: {"Authorization" : `Bearer ${passedData}`} }).then((res) => {
  ...
  const handleUpdateQuestion = () => {
    ...
    const {data} = axios.put("http:localhost:8080/Question",newQuestion,{ headers: {"Authorization" : `Bearer ${passedData}`} }).then((res) => {
  }, {withCredentials: true});
  ...
  return (
    <div className="admin-page">
      ...
      <div className="container"><Score passedData={location.state.passedData} /></div>
    </div>
  );
}
